...
function getEventData(event: EventItem) {
    if (events.identity.identityKilled.peoplePolkadotV1002006.is(event)) {
        return events.identity.identityKilled.peoplePolkadotV1002006.decode(event)
    }

    throw new UnknownVersionError(events.identity.identityKilled.name)
}

export async function identityKilled(ctx: CommonContext, block: BlockHeader, item: EventItem): Promise<undefined> {
